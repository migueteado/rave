...
for (const key in countryObject) {
  countries.push(countryObject[key]);
}

export const seedCountries = async (prismaClient: PrismaClient) => {
  const seededCountries = await prismaClient.country.createMany({
    data: countries,
    skipDuplicates: true,
  });

  console.log(`Countries Added: ${seededCountries.count}`);
